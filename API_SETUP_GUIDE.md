...
- **기능**:
  ...
  - 전체 뉴스 조회 함수 (`getAllNews`) - 언어 필터링 없음
  - 언어별 뉴스 조회 함수 (`getNews`) - 기존 호환성 유지
...
```
GET /api/v1/news?page=1&limit=50
```
  ...
  "data": [
    {
      ...
      "updatedAt": null,
      "language": "ko" // 언어 정보 (선택사항, 없으면 프론트엔드에서 자동 감지)
    }
  ]
}
```
...
프론트엔드에서는 다음과 같은 방식으로 언어별 필터링을 수행합니다:

1. **백엔드에서 언어 정보 제공하는 경우**: `language` 필드 값을 기준으로 필터링
2. **백엔드에서 언어 정보 제공하지 않는 경우**: 제목과 내용의 문자 패턴으로 언어 자동 감지
   - **한글**: `[ㄱ-ㅎ|ㅏ-ㅣ|가-힣]` 패턴 감지
   - **영어**: 다른 언어 문자가 없고 `[a-zA-Z]` 패턴이 있는 경우
   - **일본어**: `[ひらがなカタカナ一-龯]` 패턴 감지
   - **베트남어**: `[àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]` 패턴 감지
